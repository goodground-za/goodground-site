...
/**
 * Transparent over the hero photo on load; past 80px of scroll it shrinks,
 * blurs its background, and picks up a shadow. Entrance (slide down + fade)
 * plays once on mount, gated behind prefers-reduced-motion like every other
 * GSAP timeline on this page.
 */
export function HomeTestNav() {
  ...
  return (
    ...
    >
      ...
      >
        <Link href="/home-test" aria-label="GoodGround — home" className="shrink-0">
          {/* brightness-0 invert: renders the mark white in both nav states
              (transparent-over-photo and scrolled-purple), matching the comp. */}
          <Image
        ...
        <ul className="hidden items-center gap-2 lg:flex">
          {navLinks.map((link) => (
            <li key={link.href}>
              <Link
                href={link.href}
                className="font-ht-display text-ht-crimson rounded-pill inline-block bg-white px-4 py-2 text-[12px] font-bold tracking-wide uppercase transition-transform duration-200 hover:scale-[1.03]"
              >
                {link.label}
              </Link>
            </li>
          ...
          <li>
            <MagneticButton>
              {/* Matches the comp: white text on the orange fill. Flagged
                  the same as the hero's specialise pill — 3.48:1, under
                  the 4.5:1 WCAG AA bar at this size/weight. */}
              <Link
                href="/start-project"
        ...
        <button
          ...
          aria-controls="home-test-mobile-menu"
      ...
      <div
        id="home-test-mobile-menu"
        hidden={!menuOpen}
        className="bg-ht-purple mx-auto mt-2 max-w-[1600px] rounded-3xl px-6 py-4 lg:hidden"
      >
        <ul className="space-y-1">
          {navLinks.map((link) => (
            <li key={link.href}>
              <Link
                href={link.href}
                className="font-ht-display block py-3 text-lg font-bold text-white uppercase"
              >
                {link.label}
              </Link>
            </li>
