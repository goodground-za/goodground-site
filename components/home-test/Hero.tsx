...
import { heroImage } from "@/content/home-test";
...
// Same specialty list as the live homepage's hero — duplicated deliberately
// rather than exported from components/sections/Hero.tsx, since that
// component belongs to a different, unrelated design system.
const SPECIALTIES = [
  "Website Development",
  "Website Design",
  "UX Design",
    ...
    <section className="relative z-10 flex min-h-[100svh] items-center overflow-hidden rounded-b-[56px]">
      ...
      <div className="relative z-[2] mx-auto w-full max-w-[1200px] px-6 pt-24 pb-20 text-center sm:px-10 md:px-14">
        ...
        <div ref={copyRef} className="mt-8 flex flex-wrap items-center justify-center gap-3">
          <span className="font-ht-body text-[15px] font-bold text-white sm:text-[17px]">
            We specialise in:
          </span>
          {/* Matches the client's comp exactly: white text on the orange
              pill. Note this pairing is 3.48:1, under the 4.5:1 WCAG AA
              needs at this weight/size — flagged, not silently fixed,
              since it's an explicit "match the reference" instruction. */}
          <RotatingWords
            words={SPECIALTIES}
            className="bg-ht-orange rounded-pill px-5 py-2 text-[15px] font-bold text-white sm:text-[17px]"
          />
        </div>
