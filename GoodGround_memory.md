...
13. **NEVER add `Co-Authored-By: Claude … <noreply@anthropic.com>` trailers to commits in this repo.** Vercel's Hobby plan on a *private* repo checks every author AND co-author of the deployed commit against project collaborators; the Anthropic noreply address has no access, so **every git-triggered deploy is silently blocked** ("The deployment was blocked because the commit author did not have contributing access"). This bit us on 2026-07-19: four pushed commits (premium fixes + the real logo) sat on GitHub while goodground.co.za kept serving the original import build. Vercel only checks the head commit of a deploy, so one clean commit on top unblocks — no history rewrite needed. `.claude/settings.json` in this repo sets `includeCoAuthoredBy: false` as a guard, but the rule stands regardless of tooling: plain commits only, authored as Johandre (jsaayman94@gmail.com). Alternatives if it ever recurs: make the repo public (Hobby allows any authors on public repos) or upgrade Vercel to Pro.
